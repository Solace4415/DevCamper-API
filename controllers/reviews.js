...
exports.updateReview = asyncHandler(async (req, res, next) => {
  ...
  // Make sure the review belongs to user or user is an admin
  if (review.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorize to update a review ${review.id}`,
        401
      )
    );
  }
...
exports.deleteReview = asyncHandler(async (req, res, next) => {
  ...
  if (review.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorize to delete a course to course ${review.id}`,
        401
      )
    );
  }
